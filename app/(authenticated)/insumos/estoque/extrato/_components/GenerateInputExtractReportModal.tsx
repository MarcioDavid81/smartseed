...
export default function GenerateInputExtractReportModal({
  movements,
  depositName,
  productLabel,
}: Props) {
  ...
  const filterMovements = (list: Movement[]) => {
    ...
    return list.filter((m) => {
      const date = new Date(m.date as any);
  ...
  const getMovementQuantity = (m: Movement): number => {
    const direct = parseNumber((m as any).quantity);
    if (direct != null) return direct;

    const t = (m.type ?? "").toUpperCase();
    if (t === "ENTRY") return parseNumber((m as any).quantityIn) ?? 0;
    if (t === "EXIT") return parseNumber((m as any).quantityOut) ?? 0;

    const fallbackIn = parseNumber((m as any).quantityIn);
    if (fallbackIn != null) return fallbackIn;

    const fallbackOut = parseNumber((m as any).quantityOut);
    if (fallbackOut != null) return fallbackOut;

    return 0;
  };

  const getMovementBalance = (m: Movement): number | null => {
    const b = parseNumber((m as any).balance);
  ...
  const generatePDF = () => {
    ...
    const totals = sorted.reduce(
      (acc, m) => {
        ...
        const q = getMovementQuantity(m);
        if (t === "ENTRY") acc.entries += q;
        if (t === "EXIT") acc.exits += q;
        return acc;
      },
      { entries: 0, exits: 0 },
    );
    ...
    const body = sorted.map((m) => {
      const t = m.type?.toUpperCase() ?? "DESCONHECIDO";
      const balance = getMovementBalance(m);

      return [
        new Date(m.date as any).toLocaleDateString("pt-BR"),
        formatNumber(getMovementQuantity(m)),
    ...
    autoTable(doc, {
      startY: 45,
      head: [["Data", "Quantidade", "Tipo", "Saldo"]],
      showHead: "firstPage",
      body,
