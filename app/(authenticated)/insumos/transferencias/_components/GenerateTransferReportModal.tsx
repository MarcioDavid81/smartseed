...
export default function GenerateTransferReportModal() {
  ...
  const depositoOrigem = Array.from(
    new Set(transferencias.map((h) => h.originFarm.name)),
  );

  const depositoDestino = Array.from(
    new Set(transferencias.map((h) => h.destFarm.name)),
  );

  const filtered = transferencias.filter((h) => {
    const matchProduto = !produto || h.product.name === produto;
    const matchOriginFarm = !farm || h.originFarm.name === farm;
    const matchDestFarm = !farm || h.destFarm.name === farm;
    return matchProduto && matchOriginFarm && matchDestFarm;
  });
  ...
  const generatePDF = () => {
    ...
    logo.onload = () => {
      ...
      doc.text(`Depósito Origem: ${farm || "Todos"}`, 14, 40);
      doc.text(`Depósito Destino: ${farm || "Todos"}`, 14, 45);
      ...
      autoTable(doc, {
        ...
        body: filtered.map((h) => [
          ...
          h.product.name,
          h.quantity.toLocaleString("pt-BR", { minimumFractionDigits: 2 }) + ` ${h.product.unit}`,
          h.originFarm.name,
          h.destFarm.name
        ]),
        styles: {
      ...
      doc.setFontSize(9);
      doc.text("Total Transferido por Produto", 14, finalY);
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="space-y-4">
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium">Depósito de Destino</label>
          <Select
            value={farm ?? ""}
            onValueChange={(value) =>
