...
import { ListSeedStockStatementTable } from "./_components/ListSeedStockStatementTable";
...
export default async function AgricultureStockStatementPage({
  ...
  const cultivarId = searchParams?.cultivarId;
  return (
    <div className="flex flex-col w-full min-h-screen bg-found">
      <div className="min-h-screen w-full flex bg-background">
        <main className="flex-1 py-4 px-4 md:px-8 text-gray-800 overflow-x-auto min-w-0">
          ...
          <div>
            {cultivarId ? (
              <ListSeedStockStatementTable cultivarId={cultivarId} />
            ) : (
              <div className="text-sm text-muted-foreground">
                Selecione uma cultivar para visualizar o extrato.
              </div>
            )}
          </div>
